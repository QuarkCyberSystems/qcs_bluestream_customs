...
class BSStockEntry(StockController):
	...
	def __init__(self, *args, **kwargs):
		super(BSStockEntry, self).__init__(*args, **kwargs)
	...
	def get_gl_entries(self, warehouse_account):
		gl_entries = super(BSStockEntry, self).get_gl_entries(warehouse_account)
	...
	def update_job_card(self):
		...
		if(doc.job_card):
			...
			for i in range(0, len(tab)):
				for j in range(0, len(jc_tab)):
					if(tab[i].get("item_code") == jc_tab[j].get("item_code")):
						jc_tab[j].transferred_qty = tab[i].get("qty")
						item_count.append(1)
					else:
						item_count.append(0)
			jc_doc.transferred_qty = sum(item_count)
